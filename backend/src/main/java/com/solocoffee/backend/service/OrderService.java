    ...
    public Order createOrder(Order order) {
        ...
        try {
                ...
                for (OrderItem item : order.getOrderItems()) {
                    ...
                    if (bomList != null && !bomList.isEmpty()) {
                        ...
                        for (ProductBOM bom : bomList) {
                            ...
                            BigDecimal requiredMaterialQuantity = bom.getQuantity().multiply(BigDecimal.valueOf(item.getQuantity()));
                            ...
                            if (!isRawMaterialSufficient) {
                                ...
                                logger.error("制作商品ID: {} 的原料ID: {} 库存不足，所需数量: {}", 
                                           item.getProductId(), bom.getMaterialId(), requiredMaterialQuantity);
                                throw new BizException(ErrorCode.INSUFFICIENT_INVENTORY, "制作商品ID: " + item.getProductId() + " 的原料库存不足");
                            }
                        }
                    } else {
                        // 没有BOM表，使用原来的成品库存检查逻辑（兼容现有测试和旧系统）
                        boolean isInventorySufficient = inventoryService.checkInventory(item.getProductId(), BigDecimal.valueOf(item.getQuantity()));
                        ...
                        if (!isInventorySufficient) {
                            ...
                            throw new BizException(ErrorCode.INSUFFICIENT_INVENTORY, "商品ID: " + item.getProductId() + " 库存不足");
                        }
                    }
                }
            }
            ...
            if (order.getOrderItems() != null) {
                ...
                for (OrderItem item : order.getOrderItems()) {
                    logger.debug("计算商品ID: {} 的金额，数量: {}, 单价: {}, 小计: {}", 
                               item.getProductId(), item.getQuantity(), item.getPrice(), item.getSubtotal());
                    
                    // 检查商品金额是否合理
                    if (item.getPrice() == null || item.getPrice().compareTo(BigDecimal.ZERO) <= 0) {
                        logger.error("商品价格必须大于0: {}", item);
                        throw new BizException(ErrorCode.PARAMETER_ERROR, "商品价格必须大于0");
                    }
                    if (item.getSubtotal() == null || item.getSubtotal().compareTo(BigDecimal.ZERO) <= 0) {
                        logger.error("商品小计必须大于0: {}", item);
                        throw new BizException(ErrorCode.PARAMETER_ERROR, "商品小计必须大于0");
                    }
                    ...
                    if (order.getOrderItems() != null) {
                        for (OrderItem item : order.getOrderItems()) {
                            ...
                            if (bomList != null && !bomList.isEmpty()) {
                                ...
                                // 扣减成品库存
                                inventoryService.deductInventory(item.getProductId(), BigDecimal.valueOf(item.getQuantity()));
                            }
                        }
                    }
                }
